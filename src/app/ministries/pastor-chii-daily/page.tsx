...
const archive = [
  { date: '18 May 2026', title: 'Walking in the Fullness', hasAudio: true, hasVideo: true },
  { date: '17 May 2026', title: 'The Discipline of Fasting', hasAudio: true, hasVideo: false },
  { date: '16 May 2026', title: 'Faith for the Impossible', hasAudio: true, hasVideo: true },
  { date: '15 May 2026', title: 'Raised to Walk in Newness', hasAudio: false, hasVideo: false },
]
...
function Countdown() {
  ...
  return (
    <div className="bg-gradient-to-br from-[#0D1B2A] to-[#1E3A8A] rounded-2xl p-8 text-center mb-4">
      ...
      <p className="text-white/30 text-xs italic">Facebook Live every evening · 9:00 PM WAT</p>
      <button
        onClick={() => window.open('https://www.facebook.com', '_blank')}
        className="mt-4 inline-flex items-center gap-2 bg-[#1877F2] hover:bg-[#1565D8] text-white text-xs font-bold px-4 py-2 rounded-full transition-colors duration-200"
      >
        Join Live on Facebook
      </button>
      <p className="text-white/20 text-xs mt-2 text-center italic">Link updated daily before 9PM WAT</p>
    </div>
  )
}
      ...
      <section className="bg-gradient-to-b from-[#FDF6E3] to-[#F9FAFB] py-16 border-b-2 border-[rgba(184,134,11,0.2)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {/* Left — countdown + WhatsApp */}
            <div>
              <Countdown />
              <Link
                href="https://whatsapp.com/channel/0029VbB8W8k2f3ELvngFmd3W"
                target="_blank"
      ...
      <section className="bg-[#F9FAFB] py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <div className="bg-[#EBF0FA] text-[#1E3A8A] text-xs font-bold px-3 py-1 rounded-full">Tuesday, 19 May 2026</div>
            <div className="bg-[#FDF6E3] text-[#92650A] text-xs font-bold px-3 py-1 rounded-full">Episode 147</div>
          </div>

          {/* Scripture */}
          <div className="bg-[#EBF0FA] border-l-4 border-[#1E3A8A] rounded-r-2xl p-5 mb-8">
            <p className="text-[#1E3A8A] text-xs font-bold uppercase tracking-widest mb-2">Today&apos;s Scripture</p>
            <p className="font-[family-name:var(--font-heading)] text-base sm:text-lg text-[#1A1A1A] italic leading-relaxed">
              &ldquo;Though your beginning was small, yet your latter end would increase abundantly.&rdquo;
            </p>
            <p className="text-gray-500 text-xs font-bold mt-3">Job 8:7</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              ...
              <p className="text-[#374151] text-base leading-relaxed mb-4">
                There is a difference between knowing about the Holy Spirit and walking in His fullness. Many believers have acknowledged the Spirit intellectually but have not surrendered daily to His leading. Paul&apos;s instruction is not a one-time event — it is a continuous filling, a daily surrender to the One who was sent to guide us into all truth.
              </p>
              <p className="text-[#374151] text-base leading-relaxed mb-6">
                Today, make a deliberate choice: yield to the Spirit in your words, your decisions, and your responses. The filling of the Spirit produces fruit that cannot be manufactured by effort alone.
              </p>

              {/* Share buttons */}
              <div className="flex flex-wrap gap-2 mb-6">
                <button className="flex items-center gap-2 bg-[#25D366] text-white text-xs font-bold px-4 py-2 rounded-lg">
                  <FaWhatsapp size={14} /> WhatsApp
                </button>
                <button className="flex items-center gap-2 bg-[#1877F2] text-white text-xs font-bold px-4 py-2 rounded-lg">
                  <FaFacebook size={14} /> Facebook
                </button>
                <button className="flex items-center gap-2 bg-black text-white text-xs font-bold px-4 py-2 rounded-lg">
                  <FaXTwitter size={14} /> X
                </button>
                <button className="flex items-center gap-2 border border-gray-200 text-gray-500 text-xs font-bold px-4 py-2 rounded-lg hover:border-[#1E3A8A] hover:text-[#1E3A8A] transition-colors">
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Spotify', 'Apple Podcasts', 'Google Podcasts', 'RSS Feed'].map((p) => (
                    <div key={p} className="border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-bold text-gray-500 cursor-pointer hover:border-[#1E3A8A] hover:text-[#1E3A8A] transition-colors">
                      {p}
                    </div>
                  ))}
                </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-gradient-to-br from-[#1E3A8A] to-[#0D1B2A] rounded-2xl p-5">
                ...
                <p className="text-white font-bold text-sm mb-1">Walking in the Fullness of the Spirit</p>
                <p className="text-white/40 text-xs mb-4">Rev. Chijioke Igbani · Ep. 147</p>
                <div className="bg-white/10 rounded-full h-1 mb-3 overflow-hidden">
                  <div className="bg-[#B8860B] h-full w-1/3 rounded-full" />
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-white/40 text-xs">14:22 / 18:45</span>
              ...
              <div className="bg-gradient-to-br from-[#0D1B2A] to-[#1E3A8A] rounded-2xl aspect-video flex flex-col items-center justify-center gap-3">
                <div className="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center shadow-lg">
                  <Play size={20} className="text-white ml-0.5" />
                </div>
                <span className="text-white/40 text-xs">Video Devotional · YouTube</span>
              </div>
          ...
          <div className="mt-12">
            <div className="flex items-center justify-between mb-5">
              ...
              <input type="text" placeholder="Search devotionals..." className="border border-gray-200 rounded-xl px-4 py-2 text-sm outline-none focus:border-[#1E3A8A] transition-colors" />
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {archive.map((item) => (
                <div key={item.title} className="bg-white border border-gray-100 rounded-xl p-4 cursor-pointer hover:border-[#1E3A8A] hover:shadow-sm transition-all duration-200">
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {item.hasAudio && <span className="bg-[#FDF6E3] text-[#92650A] text-xs font-bold px-2 py-0.5 rounded-full">Audio</span>}
                    {item.hasVideo && <span className="bg-[#EBF0FA] text-[#1E3A8A] text-xs font-bold px-2 py-0.5 rounded-full">Video</span>}
                    {!item.hasAudio && !item.hasVideo && <span className="bg-gray-100 text-gray-400 text-xs font-bold px-2 py-0.5 rounded-full">Written</span>}
                  </div>
                </div>
              ))}
            </div>
            <div className="text-center mt-4">
              <button className="border border-gray-200 hover:border-[#1E3A8A] text-gray-500 hover:text-[#1E3A8A] text-sm font-bold px-6 py-2.5 rounded-full transition-colors duration-200">
